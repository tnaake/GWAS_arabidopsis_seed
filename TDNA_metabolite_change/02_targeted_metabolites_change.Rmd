...
cD <- data.frame(name = colnames(rep1)[cols], 
    genotype = colnames(rep1[, cols]),
    injection_number = weights_rep1$injection_vial[order(
        weights_rep1$genotype[weights_rep1$genotype %in% colnames(rep1[, cols])])])
cD$genotype <- unlist(lapply(strsplit(cD$genotype, split = "_"), function(x)
    paste(x[1], x[2], sep = "_")))
...
cD <- data.frame(name = colnames(rep2)[cols], 
    genotype = colnames(rep2[, cols]),
    injection_number = weights_rep2$number_vial[order( ######################
        weights_rep2$genotype[weights_rep2$genotype %in% colnames(rep2[, cols])])])
cD$genotype <- unlist(lapply(strsplit(cD$genotype, split = "_"), function(x)
    paste(x[1], x[2], sep = "_")))
...
```{r outlier_neg}
## rep1
outlier <- c("C24_1", "C24_2", "C24_3", "C24_4", 
    "SALK_008908_3_14", "SALK_020876_4_4")
...
## divide by weight
a_n <- MatrixQCvis::normalizeAssay(a = assay(se_rep1), 
    method = "quantile division", probs = 0.75) 
se_rep1 <- MatrixQCvis:::updateSE(se = se_rep1, assay = a_n)

a_n <- MatrixQCvis::normalizeAssay(a = assay(se_rep2), 
    method = "quantile division", probs = 0.75) 
se_rep2 <- MatrixQCvis:::updateSE(se = se_rep2, assay = a_n)
...
perform_ttest <- function(features, rep1, rep2, line) {
    ...
    ttest_rep2_col0 <- alternative_hypothesis(fit_rep_n = fit_rep2_col0, 
    ...
    cols <- c("logFC_rep1", "logFC_rep2", "adj.P.Val_rep1", "adj.P.Val_rep2", 
...
features[["Cluster_32016/Cluster_067344"]] <- rownames(pl_sel)
...
tt <- perform_ttest(features_SALK_072964, rep1_cor, rep2_cor, line = "SALK_072964_5")
print("Col0")
rmarkdown::paged_table(tt[["col0"]])
print("SALK")
rmarkdown::paged_table(tt[["salk"]])
...
print("## 477.0872/8.19")
features_SALK[["Cluster_27573/Cluster_054996"]] <- rownames(pl_sel)

print("## Cluster_28337/Cluster_057215")
pl_sel <- get_metabolite(pl = rep1, mz = 463.0882, rt = 6.94)
print("## 463.0885/7.08")
features_SALK[["Cluster_28337/Cluster_057215"]] <- rownames(pl_sel[1:2, ])
# 
# pl_sel <- get_metabolite(pl = rep1, mz = 463.0882, rt = 6.94)
...
print("531.0761/7.07")
features_SALK[["Cluster_31680/Cluster_066469"]] <- rownames(pl_sel[1, ])

print("## Cluster_35169/Cluster_075199")
pl_sel <- get_metabolite(pl = rep1, mz = 593.1515, rt = 6.43)
print("## 593.1516/6.73")
features_SALK[["Cluster_35169/Cluster_075199"]] <- rownames(pl_sel[1:2, ])
...
print("## 609.1466/6.20")
features_SALK[["Cluster_36050/Cluster_077251"]] <- rownames(pl_sel[1, ])
...
print("## 958.0318/6.19")
features_SALK[["Cluster_51012/Cluster_107244"]] <- rownames(pl_sel)

print("## Cluster_56819/Cluster_114131")
pl_sel <- get_metabolite(pl = rep1, mz = 1219.2990, rt = 6.05)
print("1219.3000/6.20")
features_SALK[["Cluster_56819/Cluster_114131"]] <- rownames(pl_sel)

tt <- perform_ttest(features = features_SALK, rep1 = rep1_cor, rep2 = rep2_cor, 
    line = "SALK_011180_5")
...
perform_ttest <- function(features, rep1, rep1_neg, rep2_neg, line) {
    ...
    ttest_rep1_neg_col0 <- alternative_hypothesis(
    ...
    ttest_rep2_neg_col0 <- alternative_hypothesis(
    ...
    ttest_rep1_neg_salk <- alternative_hypothesis(
    ...
    ttest_col0 <- dplyr::left_join(ttest_col0, mappings, by = "rep1")
