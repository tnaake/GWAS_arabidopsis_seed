## taken from http://www.zzlab.net/GAPIT/
`GAPIT.0000` <- function(){
##############################################################################################
...
##############################################################################################
`GAPIT.BIPH` <- function(x=0,h2=.5,r=.25){
...
`GAPIT.Bus`<- function(Y=NULL,CV=NULL,Z=NULL,KI=NULL,GK=NULL,GD=NULL,GM=NULL,
...
GAPIT.Circle.Manhatton.Plot <- function(
	...
	cir.chr=TRUE,
	cir.chr.h=1.3,
	chr.den.col=c("darkblue", "white", "red"),
	cir.legend=TRUE,
	cir.legend.cex=0.8,
...
`GAPIT.DP` <- function(G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
...
##############################################################################################
    ...
    #print(file.from)
    #print(file.to)
    myGenotype<-GAPIT.Genotype(G=G,GD=GD,GM=GM,KI=KI,kinship.algorithm=kinship.algorithm,PCA.total=PCA.total,SNP.fraction=SNP.fraction,SNP.test=FALSE,
...
`GAPIT.EMMAxP3D` <- function(ys,xs,K=NULL,Z=NULL,X0=NULL,CVI=NULL,CV.Inheritance=NULL,GI=NULL,GP=NULL,
...
#=============================================================================================
`GAPIT.HMP2Num` <- function(nLines=n,fileHMP="hmp.txt",fileNum="num.txt",bit=1,SNP.effect="Add",SNP.impute="Middle",heading=TRUE, Create.indicator = FALSE, Major.allele.zero = FALSE){
...
`GAPIT.HapMap` <- function(G,SNP.effect="Add",SNP.impute="Middle",heading=TRUE, Create.indicator = FALSE, Major.allele.zero = FALSE){
    #Object: To convert character SNP genotpe to numerical
    #Output: Coresponding numerical value
    #Authors: Feng Tian and Zhiwu Zhang
    # Last update: May 30, 2011
...
##############################################################################################
    ...
    if(DP$PCA.total>0&!is.null(CV))CV=GAPIT.CVMergePC(CV,DP$PC)
    if(DP$PCA.total>0&is.null(CV))CV=DP$PC
    #print(head(CV))
    
    #kinship
    if(is.null(DP$KI)&!is.null(DP$kinship.algorithm)&!DP$kinship.algorithm%in%c("FarmCPU","Blink","MLMM")){
        thisGD=GD[,-1]
        myGT=GD[,1]
      ...
      if(DP$kinship.algorithm=="Separation"&DP$PCA.total>0)
      {
        thePCA=GAPIT.PCA(X =as.matrix(GD[,-1]), taxa = GT, PC.number = DP$PCA.total,file.output=DP$file.output,PCA.total=DP$PCA.total,PCA.col=DP$PCA.col,PCA.3d=DP$PCA.3d)
        theKin= GAPIT.kinship.separation(PCs=thePCA$PCs,EV=thePCA$EV,nPCs=DP$PCA.total)
      }
      colnames(theKin)=myGT
      KI=cbind(myGT,as.data.frame(theKin)) #This require big memory. Need a way to solve it.
      rm(theKin)
...
##############################################################################################
      ...
      if(byPass) 
    {
      ...
      #maf=SS$TV$maf
          ...
          effect.est=SS$mc
       #print("@@@@@@@@")
       #print(dim(SS$Compression))
       if(DP$file.output&!is.null(SS$Compression)) GAPIT.Compression.Visualization(Compression = SS$Compression, name.of.trait = DP$name.of.trait)
       ...
       if(!Create.indicator&!is.null(effect.est)){ 
        
        GWAS.2 <- cbind(GI[,1:3], effect.est)
    ...
    print(DP$Inter.type)
    if(DP$file.output&DP$Inter.Plot) GAPIT.Interactive.Manhattan(GWAS=GWAS[,1:4],X_fre=maf,plot.type=DP$Inter.type,name.of.trait = DP$name.of.trait)
...
`GAPIT.Imputation` <- function(x,GI=NULL,impute="Middle",byRow=TRUE){
...
`GAPIT.Interactive.GS`<- function(model_store=NULL,myG=NULL,myGD=NULL,myGM=NULL,myKI=NULL,myY=NULL,myCV=NULL,rel=NULL,h2=NULL,NQTN=NULL) { 
...
`GAPIT.Interactive.Manhattan`<- function(GWAS=NULL,MAF.threshold=seq(0,0.5,.1),cutOff=0.01,DPP=50000,X_fre=NULL,plot.type=c("m","q"),name.of.trait = "Trait") {   
    ...
    MP[,4]=0
    MP[,4]=1:(nrow(MP))
    MP=matrix(as.numeric(as.matrix(MP) ) ,nrow(MP),ncol(MP))
    MP=cbind(MP,as.numeric(X_fre))
    taxa=taxa[order(MP[,2])]
    MP=MP[order(MP[,2]),]
    taxa=taxa[order(MP[,1])]
    MP=MP[order(MP[,1]),]
    numMarker=nrow(MP)
    bonferroniCutOff=-log10(cutOff/numMarker)
    MP[,3] <-  -log10(MP[,3])
    #if(max(MP[,3])<5)index_P=MP[,3]>0
    #if(max(MP[,3])<10&max(MP[,3])>=5)index_P=MP[,3]>1
    #if(max(MP[,3])<20&max(MP[,3])>=10)index_P=MP[,3]>3
    index_P=MP[,3]>0

    MP <- MP[index_P,]
    taxa=taxa[index_P]
    y.lim <- ceiling(max(MP[,3]))
    chm.to.analyze <- unique(MP[,1])
    ...
    lastbase=0
        ...
        {
            index=(MP[,1]==i)
            ticks <- c(ticks, lastbase+mean(MP[index,2]))
            MP[index,2]=MP[index,2]+lastbase
            lastbase=max(MP[index,2])
        }
    ...
    #      cex.axis=1.5, cex.lab=2, ,col=plot.color[z],type = "p",pch=1,lwd=wd,cex=s+.3,main = paste("test",sep="      "),cex.main=2.5)
    ...
    #mt_sd=SharedData$new(mt)
      ...
      p <- plot_ly(
        ...
        x = ~Position,
        y = ~P_value,
        text = ~paste("MAF: ", k, '<br>SNP:', taxa),
        #size=2*y/max(y),
        color = ~as.character(zz)
        )%>%add_lines(y=bonferroniCutOff,name = 'CutOff',color=I("green"), mode = 'lines')
...
`GAPIT.Judge`<- function(Y=Y,G=NULL,GD=NULL,KI=NULL,GM=NULL,group.to=group.to,group.from=group.from,sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,kinship.algorithm=kinship.algorithm,PCA.total=PCA.total,model="MLM",SNP.test=TRUE){
...
`GAPIT.MAF` <- function(MAF=NULL,P=NULL,E=NULL,trait="",threshold.output=.1,plot.style="rainbow"){
...
`GAPIT.Main` <- function(Y,G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,SNP.P3D=TRUE,GP=NULL,GK=NULL,
...
`GAPIT.Manhattan` <-function(GI.MP = NULL,GD=NULL,name.of.trait = "Trait",plot.type = "Genomewise",
...
`GAPIT.Mutipl.Manhattan` <- function(model_store,DPP=50000,cutOff=0.01,band=5,seqQTN=NULL,Y=NULL,GM=NULL,interQTN=NULL,plot.style="Oceanic",plot.line=FALSE){
    #Object: Make a Manhattan Plot
    #Options for plot.type = "Separate_Graph_for_Each_Chromosome" and "Same_Graph_for_Each_Chromosome"
    #Output: A pdf of the Manhattan Plot
    #Authors: Alex Lipka, Zhiwu Zhang, Meng Li and Jiabo Wang
...
for(i in 1:length(environ_name))
{
  ...
  y_filter=environ_filter[environ_filter[,4]<(0.01/(nrow(environ_filter))),]
  ...
  result=environ_result[,1:4]
    ...
    MP_store=GI.MP
        ...
        plot.color=rep(c( '#EC5f67',    '#FAC863',  '#99C794',    '#6699CC',  '#C594C5'),ceiling(ncolor/5))
            ...
            cex.axis=4, cex.lab=4, ,col=plot.color[z],axes=FALSE,type = "p",pch=mypch,lwd=wd,cex=s+.3,cex.main=4)
...
return(list(multip_mapP=result0))
} #end of GAPIT.Manhattan
#=============================================================================================
`GAPIT.Numericalization` <- function(x,bit=2,effect="Add",impute="None", Create.indicator = FALSE, Major.allele.zero = FALSE, byRow=TRUE){
#Object: To convert character SNP genotpe to numerical
#Output: Coresponding numerical value
#Authors: Feng Tian and Zhiwu Zhang
# Last update: May 30, 2011 
...
`GAPIT.PCA` <- function(X,taxa, PC.number = min(ncol(X),nrow(X)),file.output=TRUE,PCA.total=0,PCA.col=NULL,PCA.3d=FALSE){
...
#=============================================================================================
`GAPIT.Phenotype.Simulation` <- function(GD,GM=NULL,h2=.75,NQTN=10,QTNDist="normal",effectunit=1,category=1,r=0.25,CV,cveff=NULL,a2=0,adim=2){
...
#=============================================================================================
`GAPIT.Power` <- function(WS=c(1e0,1e3,1e4,1e5,1e6,1e7), GM=NULL,seqQTN=NULL,GWAS=NULL,maxOut=100,
...
`GAPIT.QQ` <- function(P.values, plot.type = "log_P_values", name.of.trait = "Trait",DPP=50000,plot.style="rainbow"){
...
`GAPIT` <- function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,K=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
...
##############################################################################################
    ...
    if(model=="CMLM")
    {
      if(!is.null(group.from)&is.null(Para$group.from))Para$group.from=group.from
      if(!is.null(group.to)&is.null(Para$group.to))Para$group.to=group.to
      if(!is.null(group.by)&is.null(Para$group.by))Para$group.by=group.by
      if(is.null(Para$group.from))Para$group.from=20
      if(is.null(Para$group.to))Para$group.to=1000000
      if(is.null(Para$group.by))Para$group.by=20
    }
    if(model=="SUPER")
    {
    ...
    GAPIT_list=list(group.from=group.from ,group.to=group.to,group.by=group.by,DPP=100000,kinship.cluster=kinship.cluster, kinship.group=kinship.group,kinship.algorithm=kinship.algorithm,                                                    
                    ...
                    SNP.MAF=SNP.MAF,FDR.Rate = FDR.Rate, SNP.FDR=SNP.FDR,SNP.permutation=SNP.permutation,SNP.CV=NULL,SNP.robust="GLM",file.from=file.from, file.to=file.to, file.total=file.total, file.fragment = 99999,file.path=NULL, 
                    ...
                    LD.chromosome=NULL,LD.location=NULL,LD.range=NULL,PCA.col=PCA.col,PCA.3d=PCA.3d,NJtree.group=NJtree.group,NJtree.type=NJtree.type,
                    sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,QC=QC,GTindex=GTindex,LD=LD,plot.bin=10^5,file.output=file.output,cutOff=cutOff, Model.selection = Model.selection,output.numerical = output.numerical,
                    output.hapmap = output.hapmap, Create.indicator = FALSE,QTN=QTN, QTN.round=1,QTN.limit=0, QTN.update=TRUE, QTN.method="Penalty", Major.allele.zero = FALSE,
                    method.GLM="fast.lm",method.sub="reward",method.sub.final="reward",method.bin="static",bin.size=c(1000000),bin.selection=bin.selection,model=model,
                    h2=h2,NQTN=NQTN,QTNDist="normal",effectunit=1,category=category,r=r,cveff=NULL,a2=0,adim=2,
                    ...
                    converge=1,iteration.output=iteration.output,acceleration=0,iteration.method="accum",PCA.View.output=PCA.View.output,Geno.View.output=Geno.View.output,plot.style="Oceanic",SUPER_GD=NULL,SUPER_GS=SUPER_GS)
    ...
    if(m==1)DP=GAPIT.DP(G=G,GD=GD,GM=GM,KI=KI,Z=Z,CV=CV,CV.Inheritance=Para$CV.Inheritance,GP=GP,GK=GK,
    ...
    #
      ...
      out$h2=SS$h2
      out$PCA=IC$PCA
      #GD=data.frame(cbind(as.data.frame(GT),as.data.frame(GD))),GI=GI
      out$GD=DP$GD
      out$GM=DP$GM
      out$KI=IC$K
      out$GM=DP$GM
    ...
    if(Para$output.numerical) write.table(GD,  "GAPIT.Genotype.Numerical.txt", quote = FALSE, sep = "\t", row.names = TRUE,col.names = NA)
    ...
    #print(tail(IC$GM))
    if(!is.null(Y))if(Para$file.output&length(model_store)*(ncol(Y)-1)>1&length(model_store)*(ncol(Y)-1)<5)
    { 
      #print("!!!!!")
      GMM=GAPIT.Mutipl.Manhattan(model_store=model_store,Y=Y,GM=IC$GM)
      #print(str(GMM$multip_mapP))
    
      GAPIT.Circle.Manhatton.Plot(band=1,r=3,GMM$multip_mapP,plot.type=c("c","q"),signal.line=1)
...
`GAPIT.ROC` <- function(t=NULL,se=NULL,Vp=1,trait="",plot.style="rainbow"){
...
`GAPIT.Report` <- function(name.of.trait=NULL,GWAS=NULL,pred=pred,ypred=NULL,tvalue=NULL,stderr=NULL,Vp=1,
...
`GAPIT.SUPER.GS`<- function(Y=Y[,c(1,trait)],G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,GK=GK,kinship.algorithm=kinship.algorithm,
                      bin.from=bin.from,bin.to=bin.to,bin.by=bin.by,inclosure.from=inclosure.from,inclosure.to=inclosure.to,inclosure.by=inclosure.by,
				        group.from=group.from,group.to=group.to,group.by=group.by,kinship.cluster=kinship.cluster,kinship.group=kinship.group,name.of.trait=traitname,
                        file.path=file.path,file.from=file.from, file.to=file.to, file.total=file.total, file.fragment = file.fragment, file.G=file.G,file.Ext.G=file.Ext.G,file.GD=file.GD, file.GM=file.GM, file.Ext.GD=file.Ext.GD,file.Ext.GM=file.Ext.GM, 
                        SNP.MAF= SNP.MAF,FDR.Rate = FDR.Rate,SNP.FDR=SNP.FDR,SNP.effect=SNP.effect,SNP.impute=SNP.impute,PCA.total=PCA.total,GAPIT.Version=GAPIT.Version,
...
`GAPIT.get.LL` <- cmpfun(function(pheno,geno=NULL,snp.pool,X0=NULL){
...
`GAPIT.kinship.ZHANG` <- function(snps,hasInbred=TRUE) {
...
#=============================================================================================
`GAPIT2` <- function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
                group.from=1000000 ,group.to=1000000,group.by=10,DPP=100000, 
                kinship.cluster="average", kinship.group='Mean',kinship.algorithm="VanRaden",                                                    
                bin.from=10000,bin.to=10000,bin.by=10000,inclosure.from=10,inclosure.to=10,inclosure.by=10,
...
`GAPIT3` <- function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,K=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
                group.from=1000000 ,group.to=1000000,group.by=10,DPP=100000, 
                kinship.cluster="average", kinship.group='Mean',kinship.algorithm="VanRaden",                                                    
                bin.from=10000,bin.to=10000,bin.by=10000,inclosure.from=10,inclosure.to=10,inclosure.by=10,
                SNP.P3D=TRUE,SNP.effect="Add",SNP.impute="Middle",PCA.total=0, SNP.fraction = 1, seed = NULL, BINS = 20,SNP.test=TRUE,
