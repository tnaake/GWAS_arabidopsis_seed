...
emma.delta.ML.LL.w.Z <- function(logdelta, lambda, etas.1, xi.1, n, etas.2.sq ) {
  ...
  #  stopifnot(length(lambda) == length(etas.1))
    return( 0.5 * (n * (log(n / (2 * pi)) - 1 - 
        log(sum(etas.1 * etas.1 / (lambda + delta)) + etas.2.sq / delta)) - 
          (sum(log(xi.1 + delta)) + (n - t) * logdelta)) )
}
...
emma.delta.ML.dLL.w.Z <- function(logdelta, lambda, etas.1, xi.1, n, etas.2.sq ) {
    ...
    etasq <- etas.1 * etas.1
    ldelta <- lambda + delta
    return( 0.5 * (n * (sum(etasq / (ldelta * ldelta)) + etas.2.sq / (delta * delta)) / 
          (sum(etasq / ldelta) + etas.2.sq / delta) - (sum(1 / (xi.1 + delta)) + (n - t) /delta)))
}
...
emma.delta.REML.LL.wo.Z <- function(logdelta, lambda, etas) {
    ...
    return( 0.5 * (nq * (log(nq / (2 * pi)) - 1 - 
        log(sum(etas * etas / (lambda + delta)))) - sum(log(lambda + delta))))
}
...
emma.delta.REML.LL.w.Z <- function(logdelta, lambda, etas.1, n, t, etas.2.sq ) {
    ...
    nq <- n - t + tq
    delta <-  exp(logdelta)
    return( 0.5 * (nq * (log(nq / (2 * pi)) - 1 - log(sum(etas.1 * etas.1 / 
        (lambda + delta)) + etas.2.sq / delta)) - (sum(log(lambda + delta)) + (n - t) * logdelta))) 
}
...
emma.REMLE <- function(y, X, K, Z = NULL, ngrids = 100, llim = -10, ulim = 10,
    ...
    if ( is.null(Z) ) {
        ...
        for( i in 1:(m - 1) ) {
            if ((dLL[i] * dLL[i + 1] < 0) && (dLL[i] > 0) && (dLL[i + 1] < 0)) {
                r <- uniroot(emma.delta.REML.dLL.wo.Z, lower = logdelta[i], 
                    upper = logdelta[i + 1], lambda = eig.R$values, etas = etas)
                optlogdelta <- append(optlogdelta, r$root)
        ...
        Lambdas <- matrix(eig.R$values,t - q,m) + matrix(delta,t - q,m,byrow = TRUE)
        Etasq <- matrix(etas.1 * etas.1,t - q,m)
        dLL <- 0.5 * delta * ((n - q) * (colSums(Etasq / (Lambdas * Lambdas)) + 
...
emma.ML.LRT <- function(ys, xs, K, Z = NULL, X0 = NULL, ngrids = 100, llim = -10, 
    ...
    if ( sum(is.na(ys)) == 0 ) {
        ...
        for(i in 1:g) {
          vrows <- !is.na(ys[i,])      
          if ( is.null(Z) ) {
              ML0[i] <- emma.MLE(ys[i, vrows], X0[vrows, , drop = FALSE], 
                  K[vrows, vrows], NULL, ngrids, llim, ulim, esp)$ML
          }
          else {
              vids <- colSums(Z[vrows,]>0)
                
              ML0[i] <- emma.MLE(ys[i, vrows], X0[vrows, , drop = FALSE], 
                  K[vids, vids], Z[vrows, vids], ngrids, llim, ulim, esp)$ML        
          }
    }
