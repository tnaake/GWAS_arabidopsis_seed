...
divide_by_weight <- function(met, weight, tissue = "seed") {
    ...
    ## determine the row indices in weights_seeds to take for finding the corresponding weight
    if (tissue=="seed") ind_to_sweep <- ifelse(ind_cut %in% weight[, "Internal.ID"], 
            match(ind_cut, weight[, "Internal.ID"]), FALSE)
    if (tissue=="leaf") ind_to_sweep <- ifelse(ind_cut %in% weight[, "NO"], 
            match(ind_cut, weight[, "NO"]), FALSE)
    ## create vector that contains the weights (and matches in order to the columns in met_new)
...
cols_seed1_neg <- which(colnames(met1) == "Sample1_Negative"):which(colnames(met1) == "Sample338_Negative")
cols_leaf_QC_neg <- which(colnames(met2) == "leaf_QC1__begin_Negative"):which(colnames(met2) == "leaf_QC6__end_Negative")
cols_leaf_neg <- which(colnames(met2) == "leaf_Sample1_Negative"):which(colnames(met2) == "leaf_Sample336_Negative")
cols_seed2_QC_neg <- which(colnames(met2) == "QC1_Negative"):which(colnames(met2) == "QC6_end_Negative")
cols_seed2_neg <- which(colnames(met2) == "Sample1_Negative"):which(colnames(met2) == "Sample338_Negative")
...
## divide by weight
met1_dw[, cols_seed1_neg] <- divide_by_weight(met1_dw[, cols_seed1_neg], weight = weights_seeds1)
met2_dw[, cols_leaf_QC_neg] <- met2_dw[, cols_leaf_QC_neg] / weight_leaf_QC
met2_dw[, cols_leaf_neg] <- divide_by_weight(met2_dw[, cols_leaf_neg], weight = weights_leaf, tissue = "leaf")
met2_dw[, cols_seed2_QC_neg] <- met2_dw[, cols_seed2_QC_neg] / weight_seed2_QC
met2_dw[, cols_seed2_neg] <- divide_by_weight(met2_dw[, cols_seed2_neg], weight = weights_seeds2)

## remove double features from met2
met2_dw_rem <- met2_dw[!duplicated(met2_dw[, "Name"]), ]

boxplot(sqrt(log2(met1_dw[,2:314] + 1)), use.cols = TRUE) ## remove 174
boxplot(sqrt(log2(met2_dw_rem[,2:309] + 1)), use.cols = TRUE)
boxplot(log2(met2_dw_rem[,310:631] + 1), use.cols = TRUE) ## remove 79

## remove 174 from seed1
met1_dw <- met1_dw[, -which(colnames(met1_dw) == "Sample174_Negative")]

## remove 79, 178 abd 327 from seed2
met2_dw_rem <- met2_dw_rem[, -which(colnames(met2_dw_rem) %in% "Sample79_Negative")]
...
normalize <- function(met, batch_list, qc=NULL) {
    ...
    smp <- data.frame(sample = colnames(met[, inds_batch]), colour = col_day, day = day)
    rownames(smp) <- smp[, 1]
    ...
    metab.med.all <- apply(mat_log, 1, median, na.rm = T) 
    mat_log_DN <- mat_log_DNtemp + metab.med.all
    
    ## sample median normalization
    ##sample.med <- apply(mat_log_DN, 2, median, na.rm=T) ## median
    ##mat_log_DMN <- sweep(mat_log_DN, 2, sample.med, FUN="-") + median(sample.med)
    ##mat_log_DMN <- apply(mat_log_DN, 2, function(x) x / quantile(x, 0.75, na.rm=TRUE))
    
    mat_log_DMN <- limma::removeBatchEffect(x = mat_log, batch = smp[, 3])
    
    rownames(smp) <- smp[, 1]
    ##colnames(mat_log_DN) <- smp[, 1]
    ##colnames(mat_log_DMN) <- smp[, 1]
...
## met1 
batch_list_met1 <- list(c("Sample1_Negative", "Sample55_Negative"), 
    c("Sample56_Negative", "Sample112_Negative"), 
    c("Sample113_Negative", "Sample168_Negative"), 
    c("Sample169_Negative", "Sample225_Negative2"), 
    c("Sample226_Negative", "Sample283_Negative"), 
    c("Sample284_Negative", "Sample338_Negative"))

## met2 seed
batch_list_seed_met2 <- list(c("Sample1_Negative", "Sample55_Negative"), 
    c("Sample56_Negative", "Sample112_Negative"), 
    c("Sample113_Negative", "Sample168_Negative"), 
    c("Sample169_Negative", "Sample225_Negative"), 
    c("Sample226_Negative", "Sample283_Negative"), 
    c("Sample284_Negative", "Sample336_Negative"))
qc_seed_met2 <- list(c("QC1_Negative"), c("QC2_end_Negative"), 
    c("QC3_begin_Negative", "QC3_end_Negative"), 
    c("QC4_begin_Negative", "QC4_end_Negative"), 
    c("QC5_begin_Negative", "QC5_end_Negative"), 
    c("QC6_begin_Negative", "QC6_end_Negative"))

## met2 leaf
batch_list_leaf_met2 <- list(c("leaf_Sample1_Negative", "leaf_Sample57_Negative"), 
    c("leaf_Sample58_Negative", "leaf_Sample118_Negative"), 
    c("leaf_Sample120_Negative", "leaf_Sample166_Negative"),
    c("leaf_Sample170_Negative", "leaf_Sample226_Negative"), 
    c("leaf_Sample227_Negative", "leaf_Sample285_Negative"), 
    c("leaf_Sample286_Negative", "leaf_Sample336_Negative"))
qc_leaf_met2 <- list(c("leaf_QC1__begin_Negative", "leaf_QC1__end_Negative"), 
    c("leaf_QC2__begin_Negative", "leaf_QC2__end_Negative"), 
    c("leaf_QC3__begin_Negative", "leaf_QC3__end_Negative"), 
    c("leaf_QC4__begin_Negative", "leaf_QC4__end_Negative"), 
    c("leaf_QC5__begin_Negative", "leaf_QC5__end_Negative"), 
    c("leaf_QC6__begin_Negative", "leaf_QC6__end_Negative"))
...
save(met1_norm, met2_seed_norm, met2_leaf_norm, 
    file="normalization_seed_leaf_dataset_two_replicates.RData")
...
rpcaNN_seed1 <- pca(t(mat_log_seed1), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455)
rpcaDN_seed1 <- pca(t(mat_log_DN_seed1), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455, maxIterations = 1500)
rpcaDMN_seed1 <- pca(t(mat_log_DMN_seed1), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455, maxIterations = 1500)

rpcaNN_seed2 <- pca(t(mat_log_seed2), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455)
rpcaDN_seed2 <- pca(t(mat_log_DN_seed2), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455, maxIterations = 1500)
rpcaDMN_seed2 <- pca(t(mat_log_DMN_seed2), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455, maxIterations = 1500)

rpcaNN_leaf2 <- pca(t(mat_log_leaf2), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455)
rpcaDN_leaf2 <- pca(t(mat_log_DN_leaf2), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455, maxIterations = 1500)
rpcaDMN_leaf2 <- pca(t(mat_log_DMN_leaf2), method = "ppca", center = TRUE, scale = 'none', nPcs = 5, seed = 3455, maxIterations = 1500)
...
pdf('PCA_Mix_sample_seed1_NN_negative.pdf', 7, 7)
...
dev.off()
pdf('PCA_Mix_sample_seed2_NN_negative.pdf', 7, 7)
...
dev.off()
pdf('PCA_Mix_sample_leaf2_NN_negative.pdf', 7, 7)
...
pdf('PCA_Mix_sample_seed1_DN_negative.pdf', 7, 7)
...
dev.off()
pdf('PCA_Mix_sample_seed2_DN_negative.pdf', 7, 7)
...
dev.off()
pdf('PCA_Mix_sample_leaf2_DN_negative.pdf', 7, 7)
...
pdf('PCA_Mix_sample_seed1_DMN_negative.pdf', 7, 7)
...
dev.off()
pdf('PCA_Mix_sample_seed2_DMN_negative.pdf', 7, 7)
...
dev.off()
pdf('PCA_Mix_sample_leaf2_DMN_negative.pdf', 7, 7)
...
# save loadings
write.table(loadings(rpcaNN_seed1), file = 'PCA-1000_Loadings_seed1_NN_negative.txt', sep = "\t", quote = FALSE)
write.table(loadings(rpcaNN_seed2), file = 'PCA-1000_Loadings_seed2_NN_negative.txt', sep = "\t", quote = FALSE)
write.table(loadings(rpcaNN_leaf2), file = 'PCA-1000_Loadings_leaf2_NN_negative.txt', sep = "\t", quote = FALSE)

write.table(loadings(rpcaDN_seed1), file = 'PCA-1000_Loadings_seed1_DN_negative.txt', sep = "\t", quote = FALSE)
write.table(loadings(rpcaDN_seed2), file = 'PCA-1000_Loadings_seed2_DN_negative.txt', sep = "\t", quote = FALSE)
write.table(loadings(rpcaDN_leaf2), file = 'PCA-1000_Loadings_leaf2_DN_negative.txt', sep = "\t", quote = FALSE)

write.table(loadings(rpcaDMN_seed1), file = 'PCA-1000_Loadings_seed1_DMN_negative.txt', sep = "\t", quote = FALSE)
write.table(loadings(rpcaDMN_seed2), file = 'PCA-1000_Loadings_seed2_DMN_negative.txt', sep = "\t", quote = FALSE)
write.table(loadings(rpcaDMN_leaf2), file = 'PCA-1000_Loadings_leaf2_DMN_negative.txt', sep = "\t", quote = FALSE)

################################################################################
## match sample numbers to ecotype.number
namesGWAS <- read.xlsx("match_accession_names_to_id.xlsx", sheetIndex = 1)

rename <- function(mat, met_names, weight, split1 = "_Negative", 
    split2 = "Sample", tissue = "seed") {
    
    ## remove columns that contain QC
    if (length(grep(colnames(mat), pattern = "QC")) > 0) mat <- mat[, -grep(colnames(mat), pattern = "QC")]
    ...
    if (tissue == "seed") {
        ind_to_genotype_map <- ifelse(ind_cut_map %in% weight[, "Internal.ID"], 
            match(ind_cut_map, weight[, "Internal.ID"]), FALSE)
        namesSamples <- unlist(lapply(strsplit(as.character(
            weight[ind_to_genotype_map, "Name"]), split = "[.]"), "[", 2))
    }
    if (tissue == "leaf") {
        ind_to_genotype_map <- ifelse(ind_cut_map %in% weight[, "NO"], 
            match(ind_cut_map, weight[, "NO"]), FALSE)
...
## Export
write.table(mat_log_DN_seed1_rename, file = 'DN_ecotypes_seed1_negative.txt', 
    sep = "\t", quote = FALSE, row.names = FALSE)
write.table(mat_log_DMN_seed1_rename, file = 'DMN_ecotypes_seed1_negative.txt', 
    sep = "\t", quote = FALSE, row.names = FALSE)

write.table(mat_log_DN_seed2_rename, file = 'DN_ecotypes_seed2_negative.txt', 
    sep = "\t", quote = FALSE, row.names = FALSE)
write.table(mat_log_DMN_seed2_rename, file = 'DMN_ecotypes_seed2_negative.txt', 
    sep = "\t", quote = FALSE, row.names = FALSE)

write.table(mat_log_DN_leaf2_rename, file = 'DN_ecotypes_leaf2_negative.txt', 
    sep = "\t", quote = FALSE, row.names = FALSE)
write.table(mat_log_DMN_leaf2_rename, file = 'DMN_ecotypes_leaf2_negative.txt', 
    sep = "\t", quote = FALSE, row.names = FALSE)
